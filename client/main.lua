...
local function OpenStable()
    ...
    TriggerServerEvent("mino-stable:AskForMyHorses")
end
...
local function SetHorseName(data)
	...
	CreateThread(function()
		...
		if (GetOnscreenKeyboardResult()) then
            ...
            TriggerServerEvent('mino-stable:BuyHorse', data, HorseName)
...
local function CloseStable()
	...
	if DadosEncoded ~= "{}" then
		TriggerServerEvent("mino-stable:UpdateHorseComponents", dados, IdMyHorse, MyHorse_entity )
	end
end
...
local function WhistleHorse()
    if SpawnplayerHorse ~= 0 then
        ...
        TriggerServerEvent('mino-stable:CheckSelectedHorse')
        Wait(100)
        InitiateHorse()
    end
end
...
RegisterNUICallback("selectHorse", function(data)
	TriggerServerEvent("mino-stable:SelectHorseWithId", tonumber(data.horseID))
end)

RegisterNUICallback("sellHorse", function(data)
	DeleteEntity(showroomHorse_entity)
	TriggerServerEvent("mino-stable:SellHorseWithId", tonumber(data.horseID))
	TriggerServerEvent("mino-stable:AskForMyHorses")
...
RegisterNetEvent('mino-stable:client:UpdadeHorseComponents', function(horseEntity, components)
    for _, value in pairs(components) do
        NativeSetPedComponentEnabled(horseEntity, value)
    end
end)

RegisterNetEvent("mino-stable:ReceiveHorsesData", function(dataHorses)
	SendNUIMessage(
		{
