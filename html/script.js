                    ...
                    if (_ != 'name') {
                        ...
                        $(`#page_shop .scroll-container .collapsible #${index} .collapsible-body #${_}`).hover(function() {                       
                            $( this ).click(function() {                        
                                ...
                                $.post('https://mino-stable/loadHorse', JSON.stringify({ horseModel: $(this).attr('id') }));
                            });                       
        ...
        for (const [ind, tab] of Object.entries(event.data.myHorsesData)) {
            ...
            $(`#page_myhorses .scroll-container .collapsible #${HorseID}`).hover(function() {  
                $( this ).click(function() {                    
                    ...
                    $.post('https://mino-stable/loadMyHorse', JSON.stringify({ IdHorse: HorseID, horseModel: HorseIdModel, HorseComp: componentsh}));
                });                         
            }, function() {});
        }
    }
...
function confirm(){
    $.post('https://mino-stable/CloseStable');
...
$(".button-right").on('click', function() {
  ...
  //  var component = text.split(' ')[0];
    ...
    $.post('https://mino-stable/'+component, JSON.stringify({ id: nValue }));
});
...
function buyHorse(Modelhor, price, isGold) {        
    ...
    if (isGold) {        
        $.post('https://mino-stable/BuyHorse', JSON.stringify({ ModelH: Modelhor, Gold: price, IsGold: isGold }));
    } else {
        $.post('https://mino-stable/BuyHorse', JSON.stringify({ ModelH: Modelhor, Dollar: price, IsGold: isGold }));    
    }    
}


function SelectHorse(IdHorse) {    
    $.post('https://mino-stable/selectHorse', JSON.stringify({ horseID: IdHorse }));    
}


function SellHorse(IdHorse) {    
    $.post('https://mino-stable/sellHorse', JSON.stringify({ horseID: IdHorse }));
